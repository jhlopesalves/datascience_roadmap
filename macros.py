...
def _load_yaml(rel_path):
    ...
    data_path = os.path.join(root, 'docs', 'data', rel_path)
...
def define_env(env):
    ...
    def render_week(week_no):
        ...
        if proj:
            p = []
            if proj.get('title'): p.append(f"**Title:** {proj['title']}")
            if proj.get('dataset'): p.append(f"**Dataset:** {proj['dataset']}")
            if proj.get('metrics'): p.append(f"**Targets/metrics:** {proj['metrics']}")
            parts.append("\n## Project\n" + "  \n".join(p) + "\n")
    ...
    def week_toc():
        ...
        for phase, items in phases.items():
            ...
            for it in items:
                ...
                lines.append(f"- [Week {it['number']:02d} — {it['title']}](/roadmap/week-{it['number']:02d}/)")
            lines.append("\n")
        return "\n".join(lines)
